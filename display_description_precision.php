...
if(isset($_REQUEST['deleteitem'])){
    include "db.php";
    echo $sql = "DELETE FROM precision_description WHERE srno=".$_REQUEST['srno'];
    // Create connection
...
// Check connection
    ...
    $sql = "SELECT * FROM precision_description WHERE billno='" . $_REQUEST['billno'] . "' ORDER BY srno";
...
// Check connection
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo $sql2 = "
            INSERT INTO `precision_description` (`srno`, `billno`, `item_name`, `quantity`, `weight`, `item_rate`, `labour`, `creation_date`) VALUES 
